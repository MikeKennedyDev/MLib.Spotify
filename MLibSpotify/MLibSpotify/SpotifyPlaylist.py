...
class SpotifyPlaylist:
    ...
    def __init__(self,
        ...
        print(f'Playlist object created for playlist {self.PlaylistName} and populated with {len(self.__all_tracks)} tracks.')
    ...
    def GetAllTracks(self, force_refresh=False):
        ...
        while True:
            endpoint = Utilities.GetPlaylistTracksEndpoint(playlist_id=self.PlaylistId,
                                                           limit=limit,
                                                           offset=offset)
            headers = {"Authorization": f"Bearer {self.__authorization.GetAccessToken()}"}
            response = requests.get(endpoint, headers=headers)
...
def GetAllUserPlaylists(authorization):
    ...
    while True:
        endpoint = Utilities.GetAllPlaylistsEndpoint(limit=10, offset=offset)
    ...
    print(f'{len(user_playlists)} playlists found.')
    return
